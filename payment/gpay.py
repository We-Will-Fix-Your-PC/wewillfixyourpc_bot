...
def get_google_root_keys(test=True):
    google_keys_url = "https://payments.developers.google.com/paymentmethodtoken/test/keys.json" \
        if test else "https://payments.developers.google.com/paymentmethodtoken/keys.json"
    ...
    google_keys = filter(lambda k: float(k["keyExpiration"]) > now if k.get("keyExpiration") else True, google_keys)
    google_keys = list(map(lambda k: load_der_public_key(base64.b64decode(k["keyValue"]), backend), google_keys))
...
def verify_intermediate_signing_key(intermediate_signing_key, root_keys):
    ...
    signedBytes = struct.pack(f"<L6sL4sL{len(key)}s", 6, b"Google", 4, b"ECv2", len(key), key)
...
def verify_signed_message(msg, sig, intermediate_key, merchant_id):
    ...
    signedBytes = struct.pack(f"<L6sL{len(merchant_id)}sL4sL{len(msg)}s", 6, b"Google", len(merchant_id),
                              merchant_id.encode('UTF-8'), 4, b"ECv2", len(msg), msg.encode('UTF-8'))
    intermediate_key.verify(base64.b64decode(sig), signedBytes, ec.ECDSA(hashes.SHA256()))
...
def verify_encrypted_message(message, ephemeral_public_key_bytes, priv_keys):
    ...
    for k in priv_keys:
        ...
        kdf = hkdf.HKDF(algorithm=hashes.SHA256(), length=512 // 8, salt=b"\0", info=b"Google", backend=backend)
        ...
        cipher = Cipher(algorithms.AES(symmetricEncryptionKey), modes.CTR(b"\0" * 16), backend)
...
def unseal_google_token(token, merchant_id="12345678901234567890", test=True):
    ...
    intermediate_key = load_der_public_key(base64.b64decode(intermediate_signing_key["keyValue"]), backend)

    try:
        verify_signed_message(google_token["signedMessage"], google_token["signature"], intermediate_key, merchant_id)
    except cryptography.exceptions.InvalidSignature:
    ...
    private_keys = settings.GPAY_TEST_PRIVATE_KEYS if test else settings.GPAY_LIVE_PRIVATE_KEYS
    private_keys = list(map(lambda k: load_pem_private_key(k, None, backend), private_keys))

    message = verify_encrypted_message(signed_message, ephemeral_public_key_bytes, private_keys)
