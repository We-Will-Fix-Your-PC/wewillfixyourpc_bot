...
def webhook(request):
    ...
    openid_config_r = requests.get("https://login.botframework.com/v1/.well-known/openidconfiguration")
    ...
    if key:
        try:
            jwt_msg = jwt.decode(authorization, key=key, algorithms=openid_algs, audience=settings.AZURE_APP_ID,
    ...
    if not authorized:
        openid_config_r = \
            requests.get("https://login.microsoftonline.com/botframework.com/v2.0/.well-known/openid-configuration")
        openid_config_r.raise_for_status()
        openid_config = openid_config_r.json()
        openid_algs = openid_config["id_token_signing_alg_values_supported"]
        ...
        for jwk in jwks:
            ...
            if key:
                try:
                    ...
                    if jwt_msg["iss"] == \
                            "https://login.microsoftonline.com/d6d49420-f39b-4df7-a1dc-d59a935871db/v2.0" or \
                            jwt_msg["iss"] == \
